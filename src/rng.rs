...
impl GameRng {
    ...
    ///
    /// `inc_timing` は、駒を何個生成した後にカウンタをインクリメントするかのパラメータ。
    /// (ゲーム内では盤面生成中に NMI が発生してカウンタがインクリメントされる。
    /// タイミングは CPU サイクルに依存するが、通常は駒が 39 または 40 個生成された直後に起こるようだ)
    pub fn gen_board(&mut self, counter: u8, inc_timing: usize) -> (Board, bool) {
        unsafe { assert_unchecked!(inc_timing <= Square::NUM) }

        // row-major (下から上の順)
        let mut pieces = ArrayVec::<Piece, { Square::NUM }>::new();
        pieces.extend(std::iter::repeat_with(|| self.gen_piece(counter)).take(inc_timing));
        pieces.extend(
            std::iter::repeat_with(|| self.gen_piece(counter.wrapping_add(1)))
                .take(Square::NUM - inc_timing),
        );
        ...
        let ok = Piece::all().all(|piece| (board.piece_count(piece) as usize) < Square::NUM / 2);
